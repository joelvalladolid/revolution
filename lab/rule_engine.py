...
def validate_intraday_signal(mc_prob_1pct: float, ema_disc: float, rsi: float, volume: float, volume_avg: float, vix: float) -> dict:
    ...
    # 1. Monte Carlo P(>1%) >= 60%
    if mc_prob_1pct < 0.60:
        reasons.append(f"MC P(>1%) = {mc_prob_1pct*100:.1f}% < 60%")
    ...
    # 3. RSI 40-65
    if not (40 <= rsi <= 65) or pd.isna(rsi):
        reasons.append(f"RSI = {rsi:.1f} fuera de rango [40, 65]" if pd.notna(rsi) else "RSI es NaN")
        
    # 4. Volumen > promedio 20 días
    if volume <= volume_avg or pd.isna(volume):
        reasons.append(f"Volumen = {volume} <= Avg = {volume_avg}")
