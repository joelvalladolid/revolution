...
def fetch_stock_data(ticker_symbol):
    ...
    try:
        from yahooquery import Ticker
        yq = Ticker(yq_ticker, asynchronous=False, validate=True)
    ...
    if not data.get("price"):
        try:
            ...
            hist_tmp = hist_tmp.dropna(subset=["Close"])
...
def fetch_stock_data_radar(ticker_symbol: str) -> dict:
    ...
    try:
        from yahooquery import Ticker
        yq = Ticker(yq_ticker, asynchronous=False, validate=False)
...
def fetch_batch_fundamentals(tickers_list: list) -> dict:
    ...
    yq_batch = None
    try:
        yq_batch = Ticker(yq_tickers, asynchronous=False, validate=False)
