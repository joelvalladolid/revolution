...
def analyze_ticker_for_today(ticker: str, regime: str, tnx_yield: float,
    ...
    try:
        ...
        if scan_mode == "MOMENTUM":
            if not pd.isna(ema_disc) and (-40.0 <= ema_disc <= -5.0):
                primary_met = True
        ...
        if scan_mode == "MOMENTUM":
            if not pd.isna(rsi_val) and (45 <= rsi_val <= 65):
        ...
        if scan_mode == "MOMENTUM":
            signal_res = {
                "signal": primary_met and (stars >= 11),
                "fundamental_ok": stars >= 11,
                "confidence": 100.0 if (primary_met and stars >= 11) else 0.0,
                "rationale": f"Momentum OK ({stars} stars)" if (primary_met and stars >= 11) else f"Falla Momentum: EMA {ema_disc:.1f}%, {stars} stars",
        ...
        if signal_res['signal']:
            try:
                ...
                if scan_mode == "MOMENTUM":
                    ...
                    req_p, req_p10 = mc_reqs.get(regime, (0.52, -0.035))
                    mc['req_p'] = req_p
                    mc['req_p10'] = req_p10
...
# ── PAGE: Data Health ──────────────────────────────────────────────────────────
    ...
    st.markdown('<p class="hero-title">🏥 Diagnóstico de Datos</p>', unsafe_allow_html=True)
    st.markdown('<p class="hero-sub">Verificación en vivo de integridad y frescura de datos en memoria</p>', unsafe_allow_html=True)
    
    if st.button("🔄 Refrescar Diagnóstico (Forzar API)", type="primary"):
        st.session_state.data_health_refresh = datetime.datetime.now().timestamp()
        
    @st.fragment(run_every="3s")
    def render_diagnostic_live():
        sid = st.session_state.session_id
        state = GLOBAL_SCANS.get(sid)
        ...
        if state and state['status'] == 'running':
            results_to_check = state.get('results', [])
            is_running = True
            st.info("🔄 Escaneo en progreso en segundo plano. Actualizando diagnóstico en tiempo real...")
        elif 'radar_results' in st.session_state and st.session_state.radar_results:
            results_to_check = st.session_state.radar_results
            
        if not results_to_check and not is_running:
            st.warning("⚠️ Ejecuta el escáner en '📡 Radar S&P 500' primero para tener datos que diagnosticar.")
            return

        if not results_to_check and is_running:
            st.info("Esperando a que el escáner encuentre las primeras señales...")
            return
        ...
        with st.spinner("Cargando datos en memoria..."):
            ...
            def check_ticker(r):
                ...
                return {
                    ...
                    "Frescos": True,
                    "Modelo": True,
                    "RSI": f"{rsi:.1f}" if isinstance(rsi, float) and not pd.isna(rsi) else "—",
                    "EMA": f"{ema:.1f}%" if isinstance(ema, float) and not pd.isna(ema) else "—",
                    "BB": f"{bb:.2f}" if isinstance(bb, float) and not pd.isna(bb) else "—",
                    "Stoch": f"{stoch:.1f}" if isinstance(stoch, float) and not pd.isna(stoch) else "—"
                }
            
            for r in results_to_check:
                diagnostic_rows.append(check_ticker(r))
                    
        total = len(diagnostic_rows)
        ok = sum(1 for r in diagnostic_rows if r["Coincide"])
        sospechosos = total - ok
        
        c1, c2, c3 = status_placeholder.columns(3)
        c1.metric("Total Analizados", total)
        c2.metric("🟢 Frescos / Coinciden", ok)
        c3.metric("🔴 Sospechosos (>0.5% diff)", sospechosos)
        
        html = '''<table class="qt" style="width:100%;"><thead><tr>
        <th>TICKER</th><th class="r">ÚLTIMO PRECIO</th><th>FUENTE</th><th>TIMESTAMP</th>
        <th class="r">PRECIO REAL (API)</th><th>COINCIDE</th><th>FRESCOS</th><th>MODELO RECALCULADO</th>
        <th class="r">RSI</th><th class="r">EMA DISC</th><th class="r">BB</th><th class="r">STOCH</th>
        </tr></thead><tbody>'''
        
        for r in diagnostic_rows:
            icon = "🟢" if r["Coincide"] else "🔴"
            diff_badge = f'<span style="color:#FF4560;font-size:10px;margin-left:6px;font-weight:700">{r["Diff"]}</span>' if r["Diff"] else ""
            html += f'''<tr>
            <td style="font-weight:700">{r["Ticker"]}</td>
            <td class="r">${r["Memoria"]:.2f}</td>
            <td>{r["Fuente"]}</td>
            <td>{r["Timestamp"]}</td>
            <td class="r">${r["Real"]:.2f}</td>
            <td>{icon}{diff_badge}</td>
            <td>✅</td>
            <td>✅</td>
            <td class="r" style="color:#00D4A0;">{r["RSI"]}</td>
            <td class="r" style="color:#00D4A0;">{r["EMA"]}</td>
            <td class="r" style="color:#00D4A0;">{r["BB"]}</td>
            <td class="r" style="color:#00D4A0;">{r["Stoch"]}</td>
            </tr>'''
        html += "</tbody></table>"
        
        table_placeholder.markdown(html, unsafe_allow_html=True)
        
        if not is_running:
            # Si ya terminó el escaneo, no necesitamos que este fragmento se siga actualizando
            pass

    render_diagnostic_live()
