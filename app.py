...
Motor cuantitativo con régimen VIX + tcnico + fundamental.
"""
...
# Suppress yfinance console spam
...
from lab.rule_engine import evaluate_signal, RULE_SETS, calculate_confidence, validate_intraday_signal
...
/* Ocultar el ttulo y toolbar del header, no los botones */
header svg + div,
...
[data-testid="stSidebar"] label { color: var(--text-secondary); }
[data-testid="stSidebar"] .stMarkdown p { color: var(--text-secondary); }
/* Streamlit generic text */
h1, h2, h3, h4, h5, h6 { color: var(--text-primary) !important; }
.stMarkdown p, .stMarkdown span { color: var(--text-primary); }
.stMarkdown code { 
...
/* Data table */
.qt { width: 100%; border-collapse: separate; border-spacing: 0; font-size: 12px; border-radius: 12px; overflow: hidden; border: 1px solid var(--border); background: var(--bg-card); }
.qt thead { background: rgba(0,0,0,0.2); }
.qt th { text-align: left; padding: 12px 16px; font-size: 10px; font-weight: 700;
         letter-spacing: 1px; text-transform: uppercase; color: var(--text-secondary);
         border-bottom: 1px solid var(--border); }
.qt th.r, .qt td.r { text-align: right; }
.qt td { padding: 12px 16px; border-bottom: 1px solid var(--border);
         color: var(--text-primary); font-family: var(--font-mono); font-size: 12px; transition: background 0.2s; }
.qt tr:last-child td { border-bottom: none; }
.qt tr:hover td { background: var(--bg-elevated); color: #fff; }
/* Input override */
...
def get_current_regime():
    try:
        ...
        # Fix: eliminar filas con Close=NaN (datos parciales del da actual)
        vix = vix.dropna(subset=['Close'])
        if vix.empty: return "CALM", 15.0, 0.0
        # Obtener el precio ms reciente (tiempo real)
        try:
...
def get_tnx_yield():
    try:
        ...
        # Fix: eliminar filas con Close=NaN
...
def get_ticker_technicals(ticker: str, start: str, end: str, preloaded_df=None):
    ...
    try:
        ...
        import traceback
        logging.error(f"DEBUG EXCEPTION in get_ticker_technicals: {e} \n {traceback.format_exc()}")
        return None, None, None
def check_earnings_within_3_days(ticker: str) -> bool:
    try:
        tk = yf.Ticker(ticker)
        dates = tk.get_earnings_dates(limit=2)
...
def calculate_proximity_score(ema_disc: float, stars: int, regime: str, scan_mode: str = "DIP", rsi: float = None) -> float:
    """
    Score de 0-100 para empresas que NO tienen seal completa.
    Refleja qu tan cerca estn de activar la seal.
    """
    if scan_mode == "MOMENTUM":
        # Tcnico: RSI entre 40-75 es momentum saludable
        if rsi is not None and not pd.isna(rsi):
            if 50 <= rsi <= 70:
                ...
                tech_score = 10.0  # an da algo de puntos si pasa el filtro
        else:
            tech_score = 0.0
        # EMA score: ema_disc negativo = encima de EMA200 (bien para momentum)
        # Ms negativo = ms por encima = mejor seal momentum
    ...
    # Componente tcnico: qu % del umbral EMA ya alcanzaron
    ema_disc_clamped = max(0.0, ema_disc)
    if ema_target > 0:
        tech_score = min(ema_disc_clamped / ema_target, 1.0) * 40  # mx 40 pts
    else:
        tech_score = 40.0
    # Componente fundamental: estrellas / total de checks (mnimo 10 en CALM)
    min_stars = 10
    fund_score = min(stars / min_stars, 1.0) * 40  # mx 40 pts
...
def get_fundamental_stars(ticker: str, tnx_yield: float, current_price: float, df=None, ind_vals=None, scan_mode: str = 'DIP') -> tuple[int, list]:
    if not FUND_AVAILABLE: return 0, [("Mdulo fundamental no disponible (modo demo)", None)]
    try:
        ...
        if total >= EXPECTED_CHECKS:
          normalized = passed
          # datos suficientes — usar raw
        elif total >= 3:
          normalized = round((passed / total) * EXPECTED_CHECKS)
        else:
          normalized = 0
          # datos insuficientes — no evaluar
        return normalized, res.get('verdicts', [])
    except Exception as e:
        return 0, [("Error de API (Yahoo bloque la conexin)", False)]
def analyze_ticker_for_today(ticker: str, regime: str, tnx_yield: float,
                   start: str, end: str, force_fundamental: bool = False, scan_mode: str = "DIP", preloaded_df=None, current_vix: float = 15.0) -> dict | None:
    try:
        if check_earnings_within_3_days(ticker):
          return None # Omitir por earnings
        ...
        current_vol = df['Volume'].iloc[-1] if 'Volume' in df.columns else 0.0
        # Filtro de Liquidez Intraday (> 5M avg)
        ...
        if atr_pct < 1.0:
          return None
               # Check Anti-Euforia (MOMENTUM)
        if scan_mode == "MOMENTUM":
            high_52w = df['High'].tail(252).max()
            low_52w = df['Low'].tail(252).min()
            range_52w = high_52w - low_52w
            if range_52w > 0:
                pct_52w = (price - low_52w) / range_52w
                if pct_52w >= 0.90 and rsi_val > 72:
                    return None # Riesgo de colapso pos-euforia
        passed_rsi = False
        passed_vol = False
        if scan_mode == "MOMENTUM":
            ...
            if not pd.isna(rsi_val) and (35 <= rsi_val <= 65):
                passed_rsi = is_close_or_met
        if vol_avg > 300000:  # reducido de 500k - incluir ms small caps liq
            passed_vol = passed_rsi
        # Para MOMENTUM, siempre correr fundamental si pas el filtro primario
        # (en DIP solo si pas volume/RSI)
        if passed_vol:
        ...
        if scan_mode == "MOMENTUM":
          # Stars mnimas en MOMENTUM: 5 (normalizado sobre /10)
          ...
          "rationale": f"Momentum OK ({stars}/10 stars)" if (primary_met and stars >= min_mom_stars) else f"Falla Momentum: EMA {ema_disc:.1f}%, {stars}/10 stars (mn {min_mom_stars})",
        ...
        if signal_res['signal']:
            try:
                ...
                horizon = 5 # Swing de 5 das para MC realista
                mc = simulate_price_paths(price, rets, horizon_days=horizon, n_simulations=10_000)
                if scan_mode == "MOMENTUM":
                    val_res = validate_intraday_signal(
                        mc_prob_1pct=mc['prob_gt_1pct'],
                        ema_disc=ema_disc,
                        rsi=rsi_val,
                        volume=current_vol,
                        volume_avg=vol_avg,
                    ...
                    if not val_res['is_valid']:
                        ...
                        signal_res['rationale'] = val_res['rationale']
                    mc['req_regime'] = regime
                else: # DIP mode
                    p_gt_2 = mc['prob_gt_2pct']
                    p10 = mc['p10']
                    mc['req_p'] = 0.35   # relajado para 5 das
                    mc['req_regime'] = "DIP"
                    if p_gt_2 < 0.35 or p10 < -0.10:  # p10 a -10% mximo de riesgo pesimista
                        signal_res['signal'] = False
        ...
        import traceback
        logging.error(f"DEBUG EXCEPTION in analyze_ticker_for_today: {e} \n {traceback.format_exc()}")
        return None
def render_regime_banner(vix_val, regime):
    ...
    activos = {"CALM": "EMA200_disc (5%)", "SLOW_BEAR": "EMA200 (8%) + BB + Stoch", "FAST_CRASH": "Solo EMA200 (12%)"}
    st.markdown(f"""
    <div class="regime-{cls.replace('r','crash').replace('a','bear').replace('c','calm')}">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div><span style="font-size:11px;font-weight:700;color:{color}">{emoji} R͉GIMEN: {regime}</span><span style="font-size:10px;color:var(--text-muted);margin-left:12px">VIX: {vix_val:.2f}</span></div>
...
def render_scanner_table(results: list, is_proximity: bool = False):
    """
    is_proximity=True  → seccin "Próximamente", etiqueta VIGILAR/MONITOREAR
    is_proximity=False → seccin señales activas, etiqueta BUY/STRONG BUY
    """
    rows = ""
    for r in results:
        ticker = r.get('ticker', '')
        ...
        # En seccin Próximamente: siempre VIGILAR, nunca BUY
        if is_proximity:
          sig_class = "W"
          sig_text  = "VIGILAR"
          score_color = "#60A5FA"
        ...
        stars_str = f"{stars}/10" if (stars > 0 or not is_proximity) else "N/D"
        mc = r.get('mc')
        if mc:
    ...
    <table class="qt">
      <thead>
        <tr>
          ...
          <th>T͉CNICO</th><th class="r">P(>2%)</th><th class="r">P90/P10</th><th class="r">VOL ANUAL</th><th>ESTADO</th>
        </tr>
      </thead>
      <tbody>{rows}</tbody>
    </table>"""
...
def render_funnel(total, downloaded, regime, passed_ema, passed_rsi, passed_vol, passed_fund, signals):
    steps = [
        ...
        ("SE͑AL COMPLETA activa",     signals,    total),
    ]
    ...
    for label, val, mx in steps:
        ...
        color = "#22c55e" if "SE͑AL" in label and val > 0 else "#3b82f6"
...
def render_individual(ticker: str, regime: str, tnx_yield: float):
    ...
    with st.spinner(f"Analizando {ticker}..."):
        res = analyze_ticker_for_today(ticker, regime, tnx_yield, start, end, force_fundamental=True, current_vix=current_vix)
    if res is None:
        st.error(f"No se pudieron obtener datos para **{ticker}**. Verifica que el ticker sea vlido.")
        return
    s = res["signal"]
    conf = s["confidence"]
    rules = RULE_SETS.get(regime, RULE_SETS["CALM"])
    ...
    badge_html = '<span class="badge-buy">✅ SE͑AL ACTIVA</span>' if s["signal"] else '<span class="badge-wait">⚠ SIN SE͑AL COMPLETA</span>'
    st.markdown(f"""
    ...
    with c1:
        ok_reg = True  # régimen siempre est disponible
    ...
    with c2:
        ...
        st.markdown(f"""
        <div class="metric-card">
          <div class="metric-label">Tcnico</div>
    ...
    req_txt = f"Mn: {fund_min} stars" if rules["requires_fundamental"] else "Opcional"
    ...
    <div class="metric-card">
      ...
      <div class="metric-value" style="color:{color}">{stars}/10 </div>
      <div class="metric-delta-neu" style="margin-bottom:8px;">{req_txt} · {'✅ Pasa' if fund_ok else ('⚠ No requerido' if not rules['requires_fundamental'] else ' Falla')}</div>
      {checks_html}
    </div>""", unsafe_allow_html=True)
    # Señales Corto Plazo
    st.markdown('<div class="section-title"> Señales de Corto Plazo (1 da)</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
      ...
      <table class="qt" style="width:100%;">
        <thead>
          ...
          <th class="r">EDGE HIST͓RICO (1D)</th>
    ...
    """, unsafe_allow_html=True)
    # Monte Carlo
    st.markdown('<div class="section-title">📈 Simulacin Monte Carlo (10,000 trayectorias · 5 das)</div>',
          unsafe_allow_html=True)
    df_hist = res["df"]
...
def render_realtime_correlation_heatmap(df_prices: pd.DataFrame) -> None:
    if df_prices.empty:
        st.warning("Datos estructurales insuficientes para evaluacin de covarianza.")
        return
    json_payload = df_prices.to_json(orient='split')
    with st.spinner("Resolviendo topologa de covarianza cruzada..."):
...
def render_vix_page():
    ...
    st.markdown('<p class="hero-sub">Histórico del VIX y clasificacin de regmenes · últimos 90 das</p>', unsafe_allow_html=True)
    ...
    c1.metric("🟦 Das CALM",      counts.get("CALM", 0))
    c2.metric("🟨 Das SLOW_BEAR", counts.get("SLOW_BEAR", 0))
    c3.metric("🟥 Das FAST_CRASH",counts.get("FAST_CRASH", 0))
    # Color-coded VIX chart
    st.markdown('<div class="section-title">VIX Histórico con Regmenes</div>', unsafe_allow_html=True)
    ...
    st.plotly_chart(fig)
    # Recent regime log
    st.markdown('<div class="section-title">Registro de Regmenes (últimos 30 das)</div>', unsafe_allow_html=True)
...
# 
def full_reset():
    """Limpia todo el estado, cachs y variables globales."""
    ...
    # 2. Limpiar cache general de Streamlit (cubre @st.cache_data en todos los mdulos)
    st.cache_data.clear()
    # 3. Limpiar lru_cache en mdulos que lo usen
    try:
        from data_fetcher import fetch_stock_data, fetch_stock_data_radar
...
# Sidebar
with st.sidebar:
    st.markdown("""    <div style="padding:18px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px;margin-bottom:16px;">    <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#7C3AED,#3B82F6);         display:flex;align-items:center;justify-content:center;font-size:18px;">⚡</div>    <div>     <div style="font-size:15px;font-weight:800;background:linear-gradient(135deg,#A78BFA,#60A5FA);
          -webkit-background-clip:text;-webkit-text-fill-color:transparent;">FRANCOTIRADOR</div>     <div style="font-size:9px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.8px;">Trading System</div>    </div>   </div>""", unsafe_allow_html=True)
    page = st.radio("Navegación", ["📡 Radar S&P 500", "💰 Paso 2 — Cuánto Comprar",
               " Análisis Individual",
               "📈 Backtest Histórico",
               "📊 Régimen de Mercado", "🔗 Correlación", "⚙ Configuración", " Diagnóstico de Datos"],
          label_visibility="collapsed")
    st.markdown("---")
    st.markdown(f"**VIX:** `{current_vix:.2f}`")
    st.markdown(f"**TNX Yield:** `{tnx_yield:.2f}%`")
    st.markdown("---")
    if st.button("🗑 Borrar Memoria y Redescargar"):
        full_reset()
...
if page == "📡 Radar S&P 500":
    ...
    scan_btn = st.button("🚀 Ejecutar Escáner (S&P 500)")
    sp500_list = fetch_sp500_tickers_wiki_v2()
    session_id = st.session_state.session_id
    def process_ticker_all_modes(ticker, current_regime, t_yield, s_date, e_date, mode_list, preloaded_df, c_vix):
        best_res = None
        for m in mode_list:
          res = analyze_ticker_for_today(ticker, current_regime, t_yield, s_date, e_date, False, m, preloaded_df, c_vix)
          ...
          res['signal_active'] = res.get('signal', {}).get('signal', False) if isinstance(res.get('signal'), dict) else False
          res['ema200_disc'] = ema_val
          if res['signal_active']:
              return res
          if best_res is None or proximity > best_res['confidence']:
              best_res = res
        return best_res
    def scan_worker(sid, tickers, current_regime, t_yield, s_date, e_date, mode_list, c_vix):
        state = GLOBAL_SCANS[sid]
        # 1. BULK FETCH T͉CNICO (Elimina 500 requests individuales)
        try:
          from data.fetcher import fetch_history
          bulk_data = fetch_history(tickers, start=s_date, end=e_date)
        except Exception:
          bulk_data = {}
        # 2. RUN WORKERS CONCURRENTE (x10 para ir a velocidad considerable sin saturar)
        with ThreadPoolExecutor(max_workers=10) as ex:
            futures = {}
            for t in tickers:
                preloaded_df = bulk_data.get(t)
                futures[ex.submit(process_ticker_all_modes, t, current_regime, t_yield, s_date, e_date, mode_list, preloaded_df, c_vix)] = t
                time.sleep(0.02) # Pequea pausa para no saturar las APIs
            for fut in as_completed(futures):
                if state['cancel']:
                    break
                state['completed'] += 1
                t = futures[fut]
                # Verificar si los datos realmente se descargaron y son suficientes
                preloaded = bulk_data.get(t)
                if preloaded is not None and len(preloaded) >= 252:
                    state['downloaded'] += 1
                try:
                    res = fut.result()
                except Exception as e:
                    logging.error(f"Error in future result for {t}: {e}")
                    res = None
                if res is None: continue
                if not pd.isna(res['ema200_disc']):
                    state['all_ema_rows'].append(res)
                dyn_th = THRESHOLDS_BY_REGIME.get(current_regime, THRESHOLDS_BY_REGIME["CALM"])["EMA200_disc"]
                if res['primary_met'] or (not pd.isna(res['ema200_disc']) and res['ema200_disc'] >= dyn_th):
                    state['passed_ema'] += 1
        ...
        if not state['cancel']:
          ...
          state['results'], 
          key=lambda x: (
          x.get('mc', {}).get('prob_gt_1pct', 0) 
          if isinstance(x.get('mc'), dict) and x.get('mc', {}).get('req_regime') == "CALM" and x.get('scan_mode') == "MOMENTUM" 
          else (x.get('mc', {}).get('prob_gt_2pct', 0) if isinstance(x.get('mc'), dict) else 0)
          ), 
          reverse=True
          )
          state['all_ema_rows'] = sorted(state['all_ema_rows'], key=lambda x: x['ema200_disc'] if not pd.isna(x['ema200_disc']) else 0, reverse=True)
    ...
    if scan_btn:
        ...
        threading.Thread(target=scan_worker, args=(session_id, sp500_list, regime, tnx_yield, start_date, end_date, modes_to_run, current_vix), daemon=True).start()
    ...
    def render_live_radar():
        ...
        if state['status'] == 'running':
            ...
            st.warning(f"⚠ Escaneo detenido por el usuario. {c}/{t} analizados.")
            render_funnel(t, state['downloaded'], regime, state['passed_ema'], state['passed_rsi'], state['passed_vol'], state['passed_fund'], state['signals_found'])
            st.session_state.radar_results = state['results']
            if state['results']:
                st.markdown(render_scanner_table(state['results']), unsafe_allow_html=True)
    ...
    if st.session_state.radar_funnel or st.session_state.radar_results or st.session_state.radar_all_ema:
        ...
        if señales_activas:
          ...
          Régimen {regime} · Mercado en valuacin extendida · Sistema en espera
          </div>
          </div>""", unsafe_allow_html=True)
        # 2. Proximos section
...
# ── PAGE: Individual ──────────────────────────────────────────────────────────
    ...
    st.markdown('<p class="hero-sub">Evala cualquier ticker con el motor de reglas + Monte Carlo</p>', unsafe_allow_html=True)
    render_regime_banner(current_vix, regime)
    ...
    if ticker_final:
        ...
        st.markdown("""        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:16px;padding:40px;text-align:center;margin-top:20px;">
          <div style="font-size:3rem;"></div>
          <h3 style="color:#e2e8f0;margin:12px 0;">Busca un ticker para analizar</h3>
          <p style="color:#64748b;">Escribe el smbolo (AAPL, MSFT) o el nombre de la empresa y pulsa Analizar o Enter</p>
        </div>""", unsafe_allow_html=True)
# ── PAGE: Régimen ─────────────────────────────────────────────────────────────
elif page == "📊 Régimen de Mercado":
    render_vix_page()
# ── PAGE: Backtest Histórico ────────────────────────────────────────────────
elif page == "📈 Backtest Histórico":
    st.markdown('<p class="hero-title">📈 Backtest Histórico (͚últimos 3 Meses)</p>', unsafe_allow_html=True)
    st.markdown('<p class="hero-sub">Simulacin día a día del escner y compra de señales Top 10</p>', unsafe_allow_html=True)
    if 'backtest_state' not in st.session_state:
        st.session_state.backtest_state = {}
    c1, c2 = st.columns([1, 4])
    with c1:
        backtest_days = st.slider("Días de Backtest", min_value=1, max_value=365, value=90, step=1)
        if st.button("🚀 Iniciar Backtest"):
          st.session_state.backtest_state = {'status': 'Iniciando...', 'progress': 0.0, 'result': None, 'error': False}
          import threading
          from lab.backtest_engine import run_historical_backtest
          from lab_tickers import fetch_sp500_tickers_wiki_v2
          tickers = fetch_sp500_tickers_wiki_v2()
          threading.Thread(target=run_historical_backtest, args=(tickers, backtest_days, 10000, 10, st.session_state.backtest_state), daemon=True).start()
    with c2:
        st.markdown('<p style="color:var(--text-muted);font-size:0.9rem;margin-top:8px;">Este proceso descargar el historial de todo el S&P 500, simular las condiciones de mercado (VIX) y evaluar las estrategias día a día.</p>', unsafe_allow_html=True)
    @st.fragment(run_every="2s")
    def render_backtest_progress():
        state = st.session_state.get('backtest_state', {})
        if not state: return
        status = state.get('status', '')
        if status not in ['completed', 'cancelled', ''] and not state.get('error', False):
            prog = state.get('progress', 0.0)
            st.progress(prog, text=status)
            ...
            if res:
                st.success("✅ Backtest completado con xito.")
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("Retorno Total", f"{res['total_return_pct']:.2f}%")
                m2.metric("Win Rate", f"{res['win_rate']:.1f}%")
                m3.metric("Trades Realizados", res['total_trades'])
                m4.metric("Capital Final", f"${res['final_equity']:,.2f}")
                import plotly.graph_objects as go
                df_eq = res['equity_curve']
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=df_eq['date'], y=df_eq['equity'], mode='lines', fill='tozeroy', line=dict(color='#00D4A0'), name='Capital'))
                fig.update_layout(title="Curva de Capital (Equity Curve)", template="plotly_dark", margin=dict(l=0,r=0,t=40,b=0), height=350)
                st.plotly_chart(fig)
                st.markdown("### 📜 Registro Completo de Trades")
                if not res['trades'].empty:
                    df_t = res['trades'].sort_values('buy_date', ascending=False).copy()
                    df_t['Ratio R/R'] = df_t.apply(lambda x: abs(x['mean_pos'] / x['mean_neg']) if x['mean_neg'] != 0 else float('inf'), axis=1)
                    
                    df_t = df_t.rename(columns={
                        'expected_value': 'E(R) 📊',
                        'prob_positive': 'P(éxito)',
                        'mean_pos': 'Ganancia media',
                        'mean_neg': 'Pérdida media'
                    })
                    
                    cols_order = ['ticker', 'buy_date', 'pnl_pct', 'E(R) 📊', 'P(éxito)', 'Ganancia media', 'Pérdida media', 'Ratio R/R', 'profit_usd']
                    cols_present = [c for c in cols_order if c in df_t.columns]
                    st.dataframe(df_t[cols_present])
                else:
                    st.info("No se realizaron operaciones en este periodo.")
...
# ── PAGE: Configuración ───────────────────────────────────────────────────────
    ...
    st.markdown('<p class="hero-title">⚙ Configuración</p>', unsafe_allow_html=True)
    st.markdown('<p class="hero-sub">Ajustes del sistema y conexin de APIs</p>', unsafe_allow_html=True)
    st.markdown("""    <div style="background:#0f172a;border:1px solid #1e293b;border-radius:16px;padding:20px;margin-top:20px;">
...
# ── PAGE: Paso 2 — Asignacin de Capital ─────────────────────────────────────
elif page == "💰 Paso 2 — Cuánto Comprar":
    st.markdown('<p class="hero-title">💰 Paso 2 — Asignacin de Capital</p>', unsafe_allow_html=True)
    st.markdown('<p class="hero-sub">Dado tu capital, el sistema calcula cunto poner en cada empresa con seal activa</p>', unsafe_allow_html=True)
    render_regime_banner(current_vix, regime)
    # Instrucciones del flujo
    st.markdown("""    <div style="background:#0f172a;border:1px solid #1e293b;border-radius:14px;padding:20px;margin-bottom:24px;">
      <p style="color:#94a3b8;margin:0;font-size:0.9rem;line-height:1.7;">
        <b style="color:#e2e8f0;">Flujo de 2 pasos:</b><br>
        <b style="color:#00D4A0;">Paso 1</b> → Ve a <b>📡 Radar S&amp;P 500</b> y ejecuta el escner para detectar señales activas.<br>
        <b style="color:#A78BFA;">Paso 2</b> → Ingresa tu capital aqu y el sistema calcula cunto comprar de cada empresa.
      </p>
    </div>""", unsafe_allow_html=True)
    # Input de capital
    ...
    with col_btn:
        ...
        calcular_btn = st.button("⚡ Calcular Asignacin")
    # Obtener señales de la sesin
    ...
    if calcular_btn or señales_activas_alloc:
        if not señales_activas_alloc:
          ...
          <div style="font-weight:700;color:#FFB800;margin-bottom:6px;">Sin señales activas en esta sesin</div>
          <div style="font-size:0.85rem;color:#64748b;">
          Ve a <b style="color:#e2e8f0;">📡 Radar S&amp;P 500</b> → ejecuta el escner primero.<br>
          Las señales activas aparecern aqu automticamente.
          </div>
          </div>""", unsafe_allow_html=True)
        else:
          ...
          # Mostrar Stop Diario
          max_loss_usd = capital_input * 0.02
          max_gain_usd = capital_input * 0.04
          st.markdown(f"""
          <div style="background:#1e293b;border-radius:10px;padding:12px 16px;margin-top:16px;margin-bottom:24px;display:flex;justify-content:space-around;font-size:0.9rem;">
          <div>🛑 <b>Stop Loss Diario (-2%):</b> <span style="color:#FF4560">-${max_loss_usd:,.0f}</span></div>
          <div>🎯 <b>Toma de Ganancias (+4%):</b> <span style="color:#00D4A0">+${max_gain_usd:,.0f}</span></div>
          </div>
          """, unsafe_allow_html=True)
          ...
          for a in allocations:
              ...
              # Precio actual para calcular nmero de acciones
              shares_str = "—"
              if not is_cash:
                  try:
          ...
          <b style="color:#94a3b8;">Metodologa:</b> Pesos por Volatilidad Inversa (Vol Parity) con cap por ticker 15%
          y cap sectorial 30%. Régimen detectado: <b style="color:#e2e8f0;">{alloc_regime}</b>.
          Las acciones aproximadas se calculan asumiendo precio de cierre del ltimo da disponible.
          <br><b style="color:#FFB800;">⚠ Esto no es asesoramiento financiero. Siempre verifica antes de operar.</b>
          </div>
          """, unsafe_allow_html=True)
    else:
        st.markdown("""        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:16px;padding:40px;text-align:center;margin-top:20px;">
          ...
          <p style="color:#64748b;">Ve a <b style="color:#e2e8f0;">📡 Radar S&amp;P 500</b>, ejecuta el escner,<br>
          y vuelve aqu para calcular cunto invertir en cada empresa detectada.</p>
        </div>""", unsafe_allow_html=True)
# ── PAGE: Data Health ──────────────────────────────────────────────────────────
elif page == " Diagnóstico de Datos":
    st.markdown('<p class="hero-title"> Diagnóstico Profundo</p>', unsafe_allow_html=True)
    st.markdown('<p class="hero-sub">Inspeccin forense de todas las capas de datos, indicadores y reglas para un ticker</p>', unsafe_allow_html=True)
    ...
    if run_diag and test_ticker:
        with st.spinner(f"Analizando {test_ticker}..."):
          ...
          st.markdown("### 1. Conexin y Datos Fundamentales (API)")
          data = fetch_stock_data(test_ticker)
          api_time = time.time() - start_time
          if not data or data.get("price") is None:
              st.error(f" Fall la conexin o no se encontraron datos para {test_ticker}.")
              st.json(data)
          else:
              is_cached = api_time < 0.1
              cache_status = "⚡ HIT (Cach)" if is_cached else " MISS (Network Fetch)"
              st.success(f"✅ Conexin exitosa. Latencia: {api_time:.3f}s [{cache_status}]")
              with st.expander("Ver RAW JSON Completo", expanded=False):
                  st.json(data)

          # Tabla de campos clave
          st.markdown("#### Campos Clave Extrados")
          key_fields = {
          ...
          # 2. Fetch Technicals
          st.markdown("### 2. Histórico y Tcnicos")
          df, ind_vals, price = get_ticker_technicals(test_ticker, start_date, end_date)
          if df is None or df.empty:
              st.error(f" No se pudo descargar el histrico de 1 ao para {test_ticker}.")
          else:
              st.success(f"✅ Histórico descargado: {len(df)} das de trading.")
          with st.expander("Ver DataFrame Histórico (últimos 5 das)", expanded=False):
              st.dataframe(df.tail())
          st.markdown("#### Indicadores Tcnicos Calculados")
          if ind_vals:
          ...
          if not FUND_AVAILABLE:
              st.error("Mdulo estrategia no disponible")
          else:
              tech_real = {}
          ...
          # Normalizar igual que get_fundamental_stars
          EXPECTED_CHECKS = 10
          if total >= EXPECTED_CHECKS:
              normalized_stars = passed
          elif total >= 3:
              normalized_stars = round((passed / total) * EXPECTED_CHECKS)
          else:
              normalized_stars = 0
          st.metric("Estrellas Fundamentales", f"{passed} / {total} (Normalizado: {normalized_stars}/10) ")
          ...
          # 4. Monte Carlo
          st.markdown("### 4. Simulacin Monte Carlo (10,000 caminos)")
          if df is not None and not df.empty:
              rets = df['Close'].pct_change().dropna()
              mc_res = simulate_price_paths(price, rets, horizon_days=5, n_simulations=10000)
              st.success("✅ Simulacin completada")
              st.table(pd.DataFrame([mc_res]))
          else:
              st.warning("No hay histrico para Monte Carlo.")
          # 5. Evaluacin de Seal (rule_engine.py)
          st.markdown("### 5. Evaluacin de Seal (rule_engine.py)")
          if ind_vals:
              eval_res = evaluate_signal(ind_vals, regime, normalized_stars)
              col1, col2 = st.columns(2)
              col1.metric("Regla de Régimen", eval_res.get('regime'))
              col1.metric("¿Seal Activa?", "✅ S" if eval_res.get('signal') else " NO")
              col2.metric("Nivel de Confianza", f"{eval_res.get('confidence', 0):.1f}%")
              col2.metric("Rationale", eval_res.get('rationale'))
              with st.expander("Detalles de la seal", expanded=True):
                  st.json(eval_res)
          else:
              st.warning("No hay ind_vals para evaluar la seal.")
