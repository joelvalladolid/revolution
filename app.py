...
_INV  = r"C:\Users\Alumno\Desktop\SMF\Invertir ESTRATEGIA SKILL MD"
for p in [_ROOT, _INV]:
...
def get_fundamental_stars(ticker: str, tnx_yield: float, current_price: float) -> tuple[int, list]:
    if not FUND_AVAILABLE: return 10, []
    try:
...
def render_scanner_table(results: list):
    rows = ""
    for r in results:
        # Check dictionary structure, user wanted: ticker, nombre, precio, ema200_disc, stars, confidence, signal
        # Map our results to this
        ticker = r.get('ticker', '')
        ...
        # ema_disc is typically negative in our engine, displaying as absolute percent or keeping negative if under EMA
        ...
        <tr>
          ...
          <td>
            <div class="sb" style="display:flex;align-items:center;gap:8px">
              ...
              <span style="font-family:var(--font-mono);font-size:11px;color:{score_color}">{conf}</span>
            </div>
          </td>
    ...
    <table class="qt">
      <thead>
        <tr>
          ...
          <th class="r">EMA200 DISC</th><th class="r">STARS</th>
          <th>CONFIDENCE</th><th>SEÑAL</th>
        </tr>
      </thead>
      <tbody>{rows}</tbody>
    </table>"""
...
with st.sidebar:
    ...
    page = st.radio("", ["📡 Radar S&P 500", "🔍 Análisis Individual",
...
if page == "📡 Radar S&P 500":
    ...
    if st.session_state.radar_funnel or st.session_state.radar_results or st.session_state.radar_all_ema:
        ...
        if st.session_state.radar_all_ema:
            ...
            st.markdown(render_scanner_table(proximos), unsafe_allow_html=True)
